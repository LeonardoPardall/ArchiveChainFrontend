...
export class FileViewerComponent implements OnInit {
  ...
  private async getContentTypeFromFileName(fileName: string, respBody: Blob): Promise<string> {
    ...
    const content = await respBody.arrayBuffer();

    const detectImage = this.detectImage(content);
    console.log('Detected image type:', detectImage);
    if(detectImage != ''){
      return detectImage;
    }
    const decoder = new TextDecoder('utf-8');
    if(this.isValidJson(decoder.decode(content))){
      return 'application/json';
    }
    // Default fallback
    return 'text/html';
}
private detectImage(buffer: ArrayBuffer): string {
  ...
  // JPEG: JFIF (FF D8 FF E0) or Exif (FF D8 FF E1)
  if (
